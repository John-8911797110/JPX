...
class ADD(Model):
    ...
    def cal_ic_metrics(self, pred, label):
        ...
        a = label.isnull().sum().sum()
        b = pred.isnull().sum().sum()
    ...
    def test_epoch(self, data_x, data_y, data_m):
        ...
        for k in keys:
            ...
            metrics[k] = np.nansum(vs) / (len(vs)-np.isnan(vs).sum())
    ...
    def predict(self, dataset: DataProcessor, segment: Union[Text, slice] = "test"):
